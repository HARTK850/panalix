...
const dom = {
    ...
    keyValidationMessage: document.getElementById('key-validation-message'), // הודעות בדיקת מפתח
    ...
    updateMainKeyBtn: document.getElementById('update-main-key-btn'), // חדש: כפתור עדכון מפתח ראשי
...
function registerEventListeners() {
    ...
    dom.updateMainKeyBtn.addEventListener('click', handleUpdateMainKey); // חדש: כפתור עדכון מפתח ראשי
}

// --- 4. ניהול API ומפתחות גיבוי ---

/**
 * מאתחל את ה-API עם מערך המפתחות ומנסה להגדיר את המפתח הנוכחי.
 * @param {string[]} keys - מערך מפתחות ה-API.
 * @param {number} [startIndex=0] - האינדקס ממנו להתחיל.
 * @returns {boolean} - האם האתחול הצליח (כלומר, נמצא מפתח עובד).
 */
function initializeApi(keys, startIndex = 0) {
    apiKeyPool = keys;
    ...
    // נסה למצוא מפתח עובד
    for (let i = startIndex; i < apiKeyPool.length; i++) {
        if (setApiKey(apiKeyPool[i])) {
            currentKeyIndex = i;
            updateKeyCountDisplay();
            return true;
        }
...
/**
 * מגדיר את מפתח ה-API הנוכחי ומאתחל את המודלים.
 * @param {string} key - מפתח ה-API.
 * @returns {boolean} - האם האתחול הצליח.
 */
function setApiKey(key) {
    try {
        if (!key) throw new Error("מפתח ריק");
        ...
        console.log(`מפתח API הוגדר בהצלחה.`);
        return true;
    } catch (error) {
...
/**
 * מנסה לבדוק את תקינות המפתח על ידי קריאה קטנה למודל.
 * @param {string} key - מפתח ה-API לבדיקה.
 * @returns {Promise<boolean>} - האם המפתח עובד.
 */
async function testApiKey(key) {
    if (!key) return false;
    try {
        // אתחול זמני לצורך בדיקה בלבד
        const tempGenAI = new GoogleGenerativeAI(key);
        const model = tempGenAI.getGenerativeModel({ model: "gemini-2.5-flash" });
        
        // קריאה קטנה ומהירה
        await model.generateContent({ contents: [{ parts: [{ text: "echo 1" }] }] });
        
        return true;
    } catch (error) {
        console.error("בדיקת מפתח נכשלה:", error);
        return false;
    }
}
...
function switchToNextKey() {
    ...
    if (currentKeyIndex < apiKeyPool.length) {
        ...
        if (setApiKey(apiKeyPool[currentKeyIndex])) {
            return true;
        } else {
            // אם המפתח הבא לא תקין, נסה את הבא אחריו
...
async function generateWithRetry(model, generationConfig, contents) {
    if (!genAI || !model) throw new Error("API לא אותחל. אנא הזן מפתח API תקין.");
...
/**
 * מטפל בשמירת מפתח API ראשי (בודק תקינות לפני השמירה)
 */
async function handleSaveApiKey() {
    const key = dom.apiKeyInput.value.trim();
    dom.keyValidationMessage.classList.remove('error', 'success');
    dom.keyValidationMessage.classList.add('hidden');
    
    if (!key) {
        showMessageBox(dom.keyValidationMessage, "אנא הזן מפתח API חוקי.", 'error');
        return;
    }

    showLoading(true, "בודק את תקינות המפתח...");

    const isKeyValid = await testApiKey(key);
    ...
    if (isKeyValid) {
        ...
        initializeApi(keys);
        dom.apiKeyInput.value = ''; // נקה את השדה
        showMessageBox(dom.keyValidationMessage, "✅ המפתח נבדק ונשמר בהצלחה! עובר למסך הראשי...", 'success');
        setTimeout(() => showScreen('main-app'), 1500);
    } else {
        // המפתח לא תקין - לא שומרים!
        showMessageBox(dom.keyValidationMessage, "❌ המפתח אינו תקין. אנא ודא שהמפתח שהוזן נכון.", 'error');
        dom.apiKeyInput.value = ''; // נקה את השדה
    }
}
...
function handleAddBackupKeys() {
    ...
    if (newKeysText) {
        ...
        if (newKeys.length > 0) {
            ...
            if (!genAI) initializeApi(apiKeyPool, currentKeyIndex);
...
/**
 * מטפל בכפתור "עדכן מפתח ראשי"
 */
function handleUpdateMainKey() {
    if (confirm("האם אתה בטוח שברצונך למחוק את המפתח הראשי השמור ולחזור למסך ההזנה?")) {
        // שמור רק את נתוני הפרויקט, מחק את המפתחות
        ...
        showMessageBox(dom.keyValidationMessage, "המפתח הראשי נמחק. אנא הזן מפתח חדש.", 'success');
    }
}
...
async function handleGeneratePlan() {
    ...
    if (!modelPlanner) {
        alert("שגיאה: מודל ה-API לא אותחל. ייתכן שיש בעיה במפתח ה-API.");
        return;
    }
...
// פונקציה זו כבר לא נחוצה, השתמש ב-saveKeysToStorage
...
/**
 * טוען את המצב הראשוני ומתקן באג טעינה במקרה של מפתח לא תקין.
 */
function loadInitialState() {
    const keys = loadKeysFromStorage();
    const proj = localStorage.getItem(STORAGE_PROJECT_KEY);
    
    // ניסיון לאתחל את ה-API עם המפתחות השמורים
    const isApiInitialized = initializeApi(keys);

    if (keys.length > 0 && isApiInitialized) {
        // המפתחות תקינים וה-API אותחל בהצלחה
        if (proj) {
            ...
            if (currentProject.generatedPages?.length > 0) {
                ...
                // הסתר את מחוון ההתקדמות אם העבודה הושלמה
        ...
        // אם אין מפתחות, או שיש מפתחות אבל אף אחד לא הצליח לאתחל את ה-API (כי המפתח הראשי לא תקין)
        showScreen('api-key-screen');
        if (keys.length > 0 && !isApiInitialized) {
            showMessageBox(dom.keyValidationMessage, "המפתח הראשי השמור אינו תקין או נגמרה לו המכסה. אנא הזן מפתח חדש.", 'error');
        }
    }
    updateStorageUsageDisplay();
}
...
/**
 * מציג הודעת מערכת מתחת לשדה הקלט.
 * @param {HTMLElement} element - אלמנט ה-DIV להצגת ההודעה.
 * @param {string} message - הטקסט להצגה.
 * @param {string} type - 'success' או 'error'.
 */
